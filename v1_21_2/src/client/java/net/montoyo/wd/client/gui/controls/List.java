...
public class List extends BasicControl {
    ...
    private void createFBO() {
        ...
        fbo = new TextureTarget(parent.screen2DisplayX(width), parent.screen2DisplayY(height), true);
    ...
    private void renderToFBO(MultiBufferSource.BufferSource source) {
        ...
        graphics.renderOutline(0, 0, width, height, 0x00000000);
        graphics.flush();
    ...
    public void draw(GuiGraphics graphics, int mouseX, int mouseY, float ptt) {
        if(visible) {
            if(update) {
                renderToFBO(Minecraft.getInstance().renderBuffers().bufferSource());
                update = false;
            }
