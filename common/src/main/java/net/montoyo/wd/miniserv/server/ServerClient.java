...
public class ServerClient extends AbstractClient {
    ...
    public ServerClient(SocketChannel chan, Selector sel) {
        super();
        socket = chan;
        selector = sel;

        try {
            selKey = chan.register(sel, SelectionKey.OP_READ, this);
        } catch(Throwable t) {
            Log.warningEx("Could not register client channel to selector", t);
        }
    ...
    public boolean hasTimedOut(long ctime) {
        return (ctime - lastDataTime) >= Constants.CLIENT_TIMEOUT;
    }

    @PacketHandler(PacketID.INIT_CONN)
    public void handleInitConn(DataInputStream dis) throws IOException {
        if(uuid != null) {
        ...
        byte[] key = Server.getInstance().getClientManager().getClientKey(clientId);

        if(key == null) {
            Log.warning("Unkown client with UUID %s wanted to connect", clientId.toString());
            remove = true;
        } else {
            this.uuid = clientId;
    ...
    public void handleAuthenticate(DataInputStream dis) throws IOException {
        if(uuid == null) {
