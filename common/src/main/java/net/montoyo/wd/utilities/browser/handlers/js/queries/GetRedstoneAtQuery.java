...
public class GetRedstoneAtQuery extends JSQueryHandler {
    public GetRedstoneAtQuery() {
        super("GetRedstoneAt");
    }
    ...
    public boolean handle(CefBrowser browser, CefFrame frame, JsonObject data, boolean persistent, CefQueryCallback callback) {
        ...
        int signal = bs.getValue(ScreenBlock.emitting) ? 0 : level.getSignal(bpos, Direction.values()[side.reverse().ordinal()]);
        callback.success("{\"level\":" + signal + "}");
        return true;
    }
}
