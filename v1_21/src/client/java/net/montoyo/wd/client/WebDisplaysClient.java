...
public class WebDisplaysClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        ScreenEvents.AFTER_INIT.register((client, screen, scaledWidth, scaledHeight) -> {
            ...
            var oldList = new java.util.ArrayList<>(screenAccessor.getChildren());
            for (var widget : oldList) {
                if (!(widget instanceof Button btn)) continue;
                int bx = btn.getX();
                int by = btn.getY();
                if (bx >= leftPos + 160 && bx <= leftPos + 190 &&
                    by >= topPos - 2 && by <= topPos + 16) {
                    screenAccessor.getChildren().remove(btn);
                    screenAccessor.getRenderables().remove(btn);
                }
            }
            ...
            screenAccessor.getRenderables().add((gui, mx, my, delta) -> {
